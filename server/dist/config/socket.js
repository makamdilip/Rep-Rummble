...
const userSockets = new Map(); // sessionId -> socket
...
function initializeSocket(httpServer) {
    ...
    chatNamespace.on('connection', (socket) => {
        ...
        socket.on('send_message', async (data) => {
            try {
                ...
                // Get conversation
                let conversation = await SupportConversation_model_1.SupportConversation.findById(conversationId);
                if (!conversation) {
                ...
                if (conversation.status === 'active') {
                    ...
                    const aiResponse = await (0, support_ai_service_1.generateSupportResponse)(content, history.map(m => ({ role: m.senderType, content: m.content })), { name: userName, email: userEmail });
                    ...
                    if (aiResponse.shouldEscalate) {
                        ...
                        const systemMessage = await SupportMessage_model_1.SupportMessage.create({
                            conversationId,
                            senderType: 'system',
                    ...
                    const aiMessage = await SupportMessage_model_1.SupportMessage.create({
                        conversationId,
                        senderType: 'ai',
    ...
    agentNamespace.on('connection', (socket) => {
        ...
        socket.on('agent_join', async (data) => {
            ...
            // Update agent status
            await SupportAgent_model_1.SupportAgent.findByIdAndUpdate(agentId, {
                status: 'online',
                isAvailable: true,
                lastActiveAt: new Date()
            });
            // Send current queue
            const queue = await SupportConversation_model_1.SupportConversation.find({
        ...
        socket.on('claim_chat', async (data) => {
            try {
                ...
                const conversation = await SupportConversation_model_1.SupportConversation.findByIdAndUpdate(conversationId, {
                    status: 'with_agent',
                ...
                socket.join(`conversation:${conversationId}`);
                // Get agent info
                const agent = await SupportAgent_model_1.SupportAgent.findById(agentId);
        ...
        socket.on('agent_message', async (data) => {
            try {
                ...
                const agent = await SupportAgent_model_1.SupportAgent.findById(agentId);
                const message = await SupportMessage_model_1.SupportMessage.create({
                    conversationId,
                    senderType: 'agent',
                    senderId: agentId,
                ...
                // Update conversation activity
                await SupportConversation_model_1.SupportConversation.findByIdAndUpdate(conversationId, {
        ...
        socket.on('agent_leave', async (data) => {
            ...
            await SupportAgent_model_1.SupportAgent.findByIdAndUpdate(data.agentId, {
                status: 'offline',
                isAvailable: false
            });
        });
