...
exports.generateExerciseWithAI = generateExerciseWithAI;
 ...
 */
async function generateExerciseWithAI(params) {
    try {
...
function getMockExerciseData(params) {
...
async function generateWorkoutPlan(params) {
    try {
        ...
        const exercises = await Exercise_model_1.Exercise.find({ isActive: true }).select('name slug category muscleGroups difficulty equipment');
        return getMockWorkoutPlan(params, exercises);
    }
    catch (error) {
        console.error('AI Workout Plan Generation Error:', error);
        // Return fallback data on error
        return getMockWorkoutPlan(params, await Exercise_model_1.Exercise.find({ isActive: true }).select('name _id'));
    }
}
function getMockWorkoutPlan(params, exercises) {
    const schedule = [];
    ...
    const workoutPlan = new WorkoutPlan_model_1.WorkoutPlan({
...
async function seedExercises() {
    ...
    for (const exercise of defaultExercises) {
        try {
            await Exercise_model_1.Exercise.findOneAndUpdate({ name: exercise.name }, { ...exercise, isActive: true }, { upsert: true, new: true });
        }
        catch (error) {
