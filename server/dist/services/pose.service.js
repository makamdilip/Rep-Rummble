...
exports.POSE_LANDMARKS = void 0;
exports.calculateAngle = calculateAngle;
exports.getJointAngles = getJointAngles;
exports.analyzeForm = analyzeForm;
exports.detectRepCompletion = detectRepCompletion;
exports.generateFormFeedback = generateFormFeedback;
 ...
 */
function getJointAngles(keypoints) {
    const angles = {};
    // Left elbow angle (shoulder -> elbow -> wrist)
    if (keypoints[exports.POSE_LANDMARKS.LEFT_SHOULDER] &&
        keypoints[exports.POSE_LANDMARKS.LEFT_ELBOW] &&
        keypoints[exports.POSE_LANDMARKS.LEFT_WRIST]) {
        angles['left_elbow'] = calculateAngle(keypoints[exports.POSE_LANDMARKS.LEFT_SHOULDER], keypoints[exports.POSE_LANDMARKS.LEFT_ELBOW], keypoints[exports.POSE_LANDMARKS.LEFT_WRIST]);
    }
    // Right elbow angle
    if (keypoints[exports.POSE_LANDMARKS.RIGHT_SHOULDER] &&
        keypoints[exports.POSE_LANDMARKS.RIGHT_ELBOW] &&
        keypoints[exports.POSE_LANDMARKS.RIGHT_WRIST]) {
        angles['right_elbow'] = calculateAngle(keypoints[exports.POSE_LANDMARKS.RIGHT_SHOULDER], keypoints[exports.POSE_LANDMARKS.RIGHT_ELBOW], keypoints[exports.POSE_LANDMARKS.RIGHT_WRIST]);
    }
    // Left knee angle (hip -> knee -> ankle)
    if (keypoints[exports.POSE_LANDMARKS.LEFT_HIP] &&
        keypoints[exports.POSE_LANDMARKS.LEFT_KNEE] &&
        keypoints[exports.POSE_LANDMARKS.LEFT_ANKLE]) {
        angles['left_knee'] = calculateAngle(keypoints[exports.POSE_LANDMARKS.LEFT_HIP], keypoints[exports.POSE_LANDMARKS.LEFT_KNEE], keypoints[exports.POSE_LANDMARKS.LEFT_ANKLE]);
    }
    // Right knee angle
    if (keypoints[exports.POSE_LANDMARKS.RIGHT_HIP] &&
        keypoints[exports.POSE_LANDMARKS.RIGHT_KNEE] &&
        keypoints[exports.POSE_LANDMARKS.RIGHT_ANKLE]) {
        angles['right_knee'] = calculateAngle(keypoints[exports.POSE_LANDMARKS.RIGHT_HIP], keypoints[exports.POSE_LANDMARKS.RIGHT_KNEE], keypoints[exports.POSE_LANDMARKS.RIGHT_ANKLE]);
    }
    // Hip angle (shoulder -> hip -> knee)
    if (keypoints[exports.POSE_LANDMARKS.LEFT_SHOULDER] &&
        keypoints[exports.POSE_LANDMARKS.LEFT_HIP] &&
        keypoints[exports.POSE_LANDMARKS.LEFT_KNEE]) {
        angles['left_hip'] = calculateAngle(keypoints[exports.POSE_LANDMARKS.LEFT_SHOULDER], keypoints[exports.POSE_LANDMARKS.LEFT_HIP], keypoints[exports.POSE_LANDMARKS.LEFT_KNEE]);
    }
    // Shoulder angle (elbow -> shoulder -> hip)
    if (keypoints[exports.POSE_LANDMARKS.LEFT_ELBOW] &&
        keypoints[exports.POSE_LANDMARKS.LEFT_SHOULDER] &&
        keypoints[exports.POSE_LANDMARKS.LEFT_HIP]) {
        angles['left_shoulder'] = calculateAngle(keypoints[exports.POSE_LANDMARKS.LEFT_ELBOW], keypoints[exports.POSE_LANDMARKS.LEFT_SHOULDER], keypoints[exports.POSE_LANDMARKS.LEFT_HIP]);
    }
    // Body straightness (for plank detection)
    if (keypoints[exports.POSE_LANDMARKS.LEFT_SHOULDER] &&
        keypoints[exports.POSE_LANDMARKS.LEFT_HIP] &&
        keypoints[exports.POSE_LANDMARKS.LEFT_ANKLE]) {
        angles['body_line'] = calculateAngle(keypoints[exports.POSE_LANDMARKS.LEFT_SHOULDER], keypoints[exports.POSE_LANDMARKS.LEFT_HIP], keypoints[exports.POSE_LANDMARKS.LEFT_ANKLE]);
    }
    return angles;
}
/**
 ...
 */
function analyzeForm(exerciseSlug, keypoints, phase) {
    ...
    if (exerciseSlug === 'squats' || exerciseSlug === 'lunges') {
        const leftKnee = keypoints[exports.POSE_LANDMARKS.LEFT_KNEE];
        const leftAnkle = keypoints[exports.POSE_LANDMARKS.LEFT_ANKLE];
        const leftHip = keypoints[exports.POSE_LANDMARKS.LEFT_HIP];
 ...
 */
function detectRepCompletion(exerciseSlug, angleHistory, threshold = 5) {
    const config = EXERCISE_FORM_CONFIGS[exerciseSlug];
    if (!config || angleHistory.length < 3)
        return false;
 ...
 */
function generateFormFeedback(analyses) {
    ...
    for (const [key, count] of sortedIssues) {
        ...
        if (percentage >= 50) {
            const relevantAnalysis = analyses.find(a => a.issues.some(i => i.bodyPart === bodyPart && i.issue === issue));
            const correction = relevantAnalysis?.issues.find(i => i.bodyPart === bodyPart && i.issue === issue)?.correction;
...
exports.default = {
    POSE_LANDMARKS: exports.POSE_LANDMARKS,
    calculateAngle,
    getJointAngles,
    analyzeForm,
    detectRepCompletion,
...
//# sourceMappingURL=pose.service.js.map