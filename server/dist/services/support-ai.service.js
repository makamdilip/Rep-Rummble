...
exports.generateAIChatResponse = generateAIChatResponse;
exports.generateSupportResponse = generateSupportResponse;
...
function getContextualQuickReplies(msg, reply) {
    const t = (msg + ' ' + reply).toLowerCase();
    if (/meal|food|nutrition|calorie|macro/.test(t))
        return ['Edit a logged meal', 'Set macro goals', 'Photo tips'];
    if (/workout|exercise|training/.test(t))
        return ['Log a workout', 'My recovery score', 'Adjust plan'];
    if (/plan|price|subscription/.test(t))
        return ['Start free trial', 'Compare plans', 'How to cancel'];
    if (/wearable|device|sync/.test(t))
        return ['Supported devices', 'Sync issues', 'Remove device'];
    if (/account|password|login/.test(t))
        return ['Reset password', 'Change email', 'Delete account'];
...
async function generateSupportResponse(query, history, _ctx) {
...
async function generateHandoffSummary(conversationHistory) {
...
exports.default = { generateAIChatResponse, generateSupportResponse, generateHandoffSummary };
